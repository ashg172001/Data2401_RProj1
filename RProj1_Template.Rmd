...
ggplot(data = dino_data, mapping = aes(x = x, y = y)) +
  geom_point()
```
Next, we jsut reuse the code for summarizing the correlation coefficient in order to find that correlation factor for the x and y values in the "circle" dataset.
```{r}
dino_data %>%
  summarize(r = cor(x, y))
```
